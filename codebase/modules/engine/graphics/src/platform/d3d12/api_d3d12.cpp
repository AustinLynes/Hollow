...
namespace Graphics {
	...
	HRESULT DirectX12API::InitilizeD3D12() {
		...
		for (UINT i = 0; i < NUM_FRAMES; i++)
		{
			...
			drawFenceEvents[i] = CreateEvent(NULL, NULL, FALSE, NULL);
	...
	void DirectX12API::Flush(ComPtr<ID3D12CommandQueue> pQueue, ComPtr<ID3D12Fence> pFence, UINT64& fenceValue, HANDLE fenceEvent)
	{
		...
		UINT64 fenceValueForSignal = Signal(pQueue, pFence, fenceValue);
		WaitForFence(pFence, fenceValue, fenceEvent);
	...
	UINT64 DirectX12API::Signal(ComPtr<ID3D12CommandQueue> queue, ComPtr<ID3D12Fence> pFence, UINT64& fenceValue)
	{
		...
		queue->Signal(pFence.Get(), finished_val);
		return finished_val;
	}
	...
	void DirectX12API::Clear(uint bit)
	{
		...
		vp_default.Width = sc.BufferDesc.Width;
		vp_default.Height = sc.BufferDesc.Height;
		...
		scissor_default.right = sc.BufferDesc.Width;
		scissor_default.bottom = sc.BufferDesc.Height;
		...
		size_t size = device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);
		CD3DX12_CPU_DESCRIPTOR_HANDLE dsv(dsv_descriptorHeap->GetCPUDescriptorHandleForHeapStart());
		dsv.Offset(size * current_frame_idx);
	...
	void DirectX12API::Draw(uint vertex_count)
	{
		ComPtr<ID3D12GraphicsCommandList> cmd = drawLists[current_frame_idx];
		cmd->DrawInstanced(vertex_count, 1, 0, 0);
	}
