...
const float3 center(aabb_t aabb)  { return (aabb.min + aabb.max) * 0.5f; };

const float3 extents(aabb_t aabb) { return  aabb.max - center(aabb); };

const aabb_t create_aabb(aabb_t a, aabb_t b) {
	
	float min_x = std::min(a.min.x, b.min.x);
	float min_y = std::min(a.min.y, b.min.y);
	float min_z = std::min(a.min.z, b.min.z);
	
	float max_x = std::max(a.max.x, b.max.x);
	float max_y = std::max(a.max.y, b.max.y);
	float max_z = std::max(a.max.z, b.max.z);


	return { {min_x, min_y, min_z,}, {max_z, max_y, max_z} };
}
